...
export async function execute(interaction) {
  ...
  const reason =
    (await interaction.options.getString("grund")) || "Kein Grund angegeben";
  ...
  const userDmEmbed = new EmbedBuilder()
    ...
    .setDescription(
      `Du wurdest vom Server **${interaction.guild.name}** gebannt. Du kannst hier einen entbannungsantrag stellen`,
    )
    .addFields(
      { name: "Moderator", value: `<@${interaction.user.id}>` },
      { name: "Grund", value: reason },
